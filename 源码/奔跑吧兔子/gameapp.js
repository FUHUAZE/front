...
function butEventHandler(G, O) {
	switch (G) {
		...
		case "moreGamesPause":
			playSound("click");
			Play68.goHome();
			// window.open("http://www.yiv.com/?utm_source=unknown&utm_medium=logo&utm_campaign=game-Hop-Dont-Stop");
			break;
		case "startGame":
			playSound("click");
			userInput.removeHitArea("startGame");
...
function initGameEnd() {
	gameState = "gameOver";
	updateShare(oGameData.curGems);
	Play68.setRankingScoreDesc(oGameData.curGems);
	if (audioType == 1) {
		musicTween.kill();
